...
export function ProfilePage({ api, profile, setProfile, onLogout }) {
  ...
  return <><PageHeader eyebrow="个人中心" title="个人资料" />{state.loading && !data ? <Loading /> : state.error ? <ErrorState message={state.error} onRetry={reload} /> : data ? <div className="profile-layout"><section className="profile-hero"><span>{(data.name || '我').slice(0, 1)}</span><div><h2>{data.name}</h2><p>学号 {data.student_no}</p></div></section><section className="paper-card profile-facts"><dl><div><dt>年级</dt><dd>{data.grade}</dd></div><div><dt>班级</dt><dd>{data.class_name}</dd></div><div><dt>登录账号</dt><dd>{data.username}</dd></div></dl><button className="row-button" onClick={() => navigate('/change-password')}>修改密码 <span>›</span></button><button className="row-button danger" onClick={onLogout}>退出登录 <span>›</span></button></section></div> : null}</>;
}
