  ...
  return <div className="editor-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <form className="course-editor" onSubmit={submit}>
      ...
      <div className="form-grid">
        ...
        <label><span>当前状态</span><div className="readonly-status">{{ DRAFT: '待完善', OPEN: '项目启用', CLOSED: '暂停使用', FINISHED: '课程已结束', ARCHIVED: '历史项目' }[form.status] || form.status}<small>启用或暂停请在项目列表操作</small></div></label>
      ...
      <section className="editor-section"><div className="section-heading"><div><strong>上课时间与场地</strong><span>保存时会检查教师、场地和已经分班学生的课表冲突；整馆与分区也不能同时占用</span></div><button type="button" onClick={() => setForm((current) => ({ ...current, schedules: [...current.schedules, { time_slot_id: '', venue_id: '' }] }))}>继续添加上课时间</button></div>{form.schedules.length ? <div className="schedule-editor-list">{form.schedules.map((item, index) => <div key={index}><b>第 {index + 1} 节安排</b><select aria-label={`第 ${index + 1} 节上课时间`} value={item.time_slot_id} onChange={(event) => changeSchedule(index, 'time_slot_id', event.target.value)}><option value="">选择上课时间</option>{meta.time_slots.map((slot) => <option key={slot.id} value={slot.id}>{slot.name}</option>)}</select><select aria-label={`第 ${index + 1} 节上课场地`} value={item.venue_id} onChange={(event) => changeSchedule(index, 'venue_id', event.target.value)}><option value="">选择上课场地</option>{meta.venues.map((venue) => <option key={venue.id} value={venue.id}>{venue.name}</option>)}</select><button type="button" aria-label={`删除第 ${index + 1} 节安排`} onClick={() => removeSchedule(index)}>删除这节安排</button></div>)}</div> : <p className="helper-text">还没有排课。项目会先保存为“尚未启用”，之后可以再安排教师、时间和场地。</p>}</section>

      <section className="editor-section"><div className="section-heading"><div><strong>适用学生范围</strong><span>设置这个体育项目可用于哪些年级或班级；学生实际可选项目由教学组统一决定</span></div></div><div className="segmented scope-tabs"><button type="button" className={form.allowed_scope.type === 'all' ? 'active' : ''} onClick={() => setScopeType('all')}>全体学生</button><button type="button" className={form.allowed_scope.type === 'grades' ? 'active' : ''} onClick={() => setScopeType('grades')}>指定年级</button><button type="button" className={form.allowed_scope.type === 'classes' ? 'active' : ''} onClick={() => setScopeType('classes')}>指定班级</button></div>{form.allowed_scope.type !== 'all' ? <div className="option-grid compact">{scopeOptions.map((item) => { const key = form.allowed_scope.type === 'grades' ? 'grades' : 'classes'; return <label className="check-card" key={item.id}><input type="checkbox" checked={(form.allowed_scope[key] || []).includes(item.id)} onChange={() => toggleScope(item.id)} /><span>{item.name}</span></label>; })}</div> : null}</section>
