  ...
  async function runImport() {
    ...
    for (const item of preview.rows) {
      try {
        ...
        if (existing) await api.updateAdminCourse(existing.id, { ...existing, ...item });
        else await api.createAdminCourse({ category_id: meta.categories[0]?.id, ...item });
        saved += 1;
      } catch (error) { failed.push({ name: item.name, message: error.message }); }
    }
    setFailures(failed); setImporting(false); onImported();
    toast(failed.length ? `已保存 ${saved} 门，${failed.length} 门因资料问题未保存` : `已导入 ${saved} 门课程`, failed.length ? 'error' : 'success');
  }

  return <details className="course-import-panel">
    <summary><strong>Excel 批量导入</strong><span>一次导入多门课程</span></summary>
    <div className="course-import-body"><span>必填：课程名称、课程容量。选填：课程分类、任课教师、课程状态、课程介绍。教师直接填写姓名，无需先创建账号。</span><label className="upload-button secondary-upload">选择课程 Excel<input type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={chooseFile} /></label></div>
    {preview ? <div className="course-import-preview"><div><strong>{fileName}</strong><span>{preview.rows.length} 门课程 · {preview.errors.length} 行需修正</span></div>{preview.errors.length ? <div className="import-errors">{preview.errors.slice(0, 6).map((error) => <span key={`${error.row_number}-${error.message}`}>第 {error.row_number} 行：{error.message}</span>)}</div> : null}<button className="primary-compact" disabled={importing || !preview.rows.length} onClick={runImport}>{importing ? '正在导入课程…' : '确认导入课程'}</button></div> : null}
