...
export function buildEnrollmentSummarySheet(courses = [], generatedAt = new Date()) {
  ...
  const rows = courses.map((course, index) => {
    ...
    return [
      ...
      textCell(course.schedules?.map((item) => item.slot_name).filter(Boolean).join('；') || '学校统一安排', { wrap: true }),
      textCell(course.schedules?.map((item) => item.venue_name).filter(Boolean).join('；') || '学校统一安排', { wrap: true }),
      numberCell(capacity),
