...
export function createApiClient({ baseUrl = '', fetchImpl = globalThis.fetch, sessionStore, onUnauthorized } = {}) {
  ...
  return {
    ...
    createAdminCourse: (payload) => request('/api/admin/courses', { method: 'POST', body: payload }),
    updateAdminCourse: (id, payload) => request(`/api/admin/courses/${id}`, { method: 'PUT', body: payload }),
