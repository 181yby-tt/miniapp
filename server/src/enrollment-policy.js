...
function courseEnrollmentScope(db, course) {
  ...
  if (scope.type === 'groups') return scope;
  const groups = (db.teaching_group_courses || []).filter((row) => row.course_id === course.id).map((row) => row.group_id);
  if (!groups.length) return scope;
  if (scope.type === 'all') return { type: 'groups', groups };
  ...
  return { type: 'classes', classes: classes.filter((item) => scope.type === 'classes' ? scope.classes?.map(Number).includes(item.id) : scope.type === 'grades' && scope.grades?.map(Number).includes(item.grade_id)).map((item) => item.id) };
}
...
function enrollmentPolicy(db, student, course, { now = Date.now(), staff = false } = {}) {
  ...
  if (scope && scope.type !== 'all' && !(scope.type === 'grades' && scope.grades?.map(Number).includes(student.grade_id)) && !(scope.type === 'classes' && scope.classes?.map(Number).includes(student.class_id)) && !(scope.type === 'groups' && db.teaching_group_classes.some((row) => scope.groups?.map(Number).includes(row.group_id) && row.class_id === student.class_id))) return deny('STUDENT_SCOPE_MISMATCH', '该课程不在你的可报名范围内');
